...
/**
 * Generic job specific fields and handling routines. <br>
 * <br>
 * NOTES:<br>
 * 1. Generic jobs must have commands that reside in /nfsvol/nls/bin on our
 * internal computer systems. Specified via the Batch_Command field. For
 * example, 'setField("Batch_Command", "MTI -opt1_DCMS -E")'. <br>
 * <br>
 * 2. Commands for the Generic jobs must NOT have ".." embedded in their path
 * <br>
 * <br>
 * 3. Generic jobs require validation where the Scheduler expects an "<< EOT >>"
 * end of result marker so it can verify it received a complete result. All of
 * the SKR utilities require the addition of the "-E" option for this purpose.
 * <br>
 * <br>
 * 4. Generic jobs also allow you to specify any special environment<br>
 * variables you might need inorder to run your command. Specified via the
 * Batch_Env field. For example, 'setField("Batch_Env",
 * "NLS=/nfsvol/nls#CC=gcc")'. Where each environment variable is separated by a
 * "#". Note that these are not needed to run the MTI command in the above
 * example, but are here for illustrative purposes only.
 *
 * Original Author: Jim Mork (September 18, 2006) Updated by Willie Rogers to
 * allow for CAS authentication
 *
...
public class GenericObject {
  ...
  public final String casAuthServer = System.getProperty("skrapi.cas.serverurl",
      "https://utslogin.nlm.nih.gov/cas/v1/tickets");

  /** url of skr api service, property: skrapi.serviceurl */
  public final String service = System.getProperty("skrapi.serviceurl",
      "https://ii.nlm.nih.gov/cgi-bin/II/UTS_Required/API_batchValidationII.pl");

  /**
   * url of skr Interactive MetaMap api service, property:
   * skrapi.servicemminterurl
   */
  public final String serviceMMInter = System.getProperty("skrapi.servicemminterurl",
   ...
   */
  public final String serviceSRInter = System.getProperty("skrapi.servicesrinterurl",
  ...
  /**
   * authenticator class name, property: nls.service.authenticator, default get
   * username and password from console : @see gov.nih.nls.util.ConsoleAuthImpl
   * see also java.net.Authenticator and java.net.PasswordAuthentication
   */
  private String authenticatorClassName = System.getProperty("nls.service.authenticator",
      "gov.nih.nlm.nls.util.ConsoleAuthImpl");
  ...
  public final static int ticketTimeout = Integer.parseInt(System.getProperty("skrapi.cas.ticket.timeout", "0"));
  ...
  public GenericObject() {
    ...
    this.serviceTicket = CasAuth.getTicket(casAuthServer, this.pa.getUserName(), new String(this.pa.getPassword()),
        this.privService);
    this.ticketTimeStamp = Calendar.getInstance();
    this.initFields();
    try {
      this.formMap.put("RUN_PROG", new StringBody("GENERIC_V", "text/plain", Charset.forName("UTF-8")));
      this.formMap.put("SKR_API", new StringBody("true", "text/plain", Charset.forName("UTF-8")));
      this.formMap.put("Batch_Command", new StringBody("skr", "text/plain", Charset.forName("UTF-8")));
   ...
   *
   * This constructor sets up a Generict with Validation Batch job request without
   * prompting the user for the username/password information. NOTE: Care should
   * be taken when using this since your authentication information is available
   * in the code!!
   *
   * @param username
   * @param password
  ...
  public GenericObject(String apikey) {
    this.privService = service;
    this.serviceTicket = CasAuth.getTicket(casAuthServer, apikey, this.privService);
    this.ticketTimeStamp = Calendar.getInstance();
    this.initFields();
    this.apikey = apikey;
    try {
   ...
   * Creates a new GenericObject object using the specified information.
   *
   * This constructor sets up a Generict with Validation Interactive job request
   *
   ...
   *
   * This constructor sets up a Generict with Validation Interactive job request
   * without prompting the user for the username/password information. NOTE: Care
   * should be taken when using this since your authentication information is
   ...
   */
  String printEntity(HttpEntity respEntity) throws IOException {
      ...
      if (addressBody instanceof StringBody) {
        if (((StringBody) addressBody).getContentLength() == 0) {
          ...
          try {
            ...
            BufferedReader reader = new BufferedReader(((StringBody) addressBody).getReader());
            String line;
  ...
  public boolean validEmail() {
    return this.formMap.containsKey("Email_Address") && this.emailIsWellFormed(this.formMap.get("Email_Address"));
  }
   ...
   public String handleSubmission() {
    ...
    try {
      ...
      if(apikey.length()==0)
        ...
        this.serviceTicket = CasAuth.getTicket(casAuthServer, this.apikey , this.privService);
      this.ticketTimeStamp = Calendar.getInstance();
      if (this.validEmail()) {
  ...
  public void initFields() {
    ...
    try {
      this.formMap.put("SKR_API", new StringBody("true", "text/plain", Charset.forName("UTF-8")));
      this.formMap.put("Batch_Command", new StringBody("", "text/plain", Charset.forName("UTF-8")));

      this.formMap.put("Batch_Env", new StringBody("", "text/plain", Charset.forName("UTF-8")));
    } catch (UnsupportedEncodingException e) {
      throw new RuntimeException(e);
    }
  } // initFields
  ...
  public void setField(String fieldName, String fieldValue) {
    try {
      this.formMap.put(fieldName, new StringBody(fieldValue, "text/plain", Charset.forName("UTF-8")));
    } catch (UnsupportedEncodingException e) {
      throw new RuntimeException(e);
    }
  } // setField
  ...
  public void setField(String fieldName, boolean fieldState) {
    try {
      this.formMap.put(fieldName, new StringBody(Boolean.toString(fieldState), "text/plain", Charset.forName("UTF-8")));
    } catch (UnsupportedEncodingException e) {
      throw new RuntimeException(e);
    }
  } // setField
  ...
  /**
   * Set file field of the requested field/option for this job using in-memory
   * buffer.
   *
   * @param fieldName      Name of the field to be updated
