...
public class GenericBatchNew {
  ...
  public static void printHelp() {
    ...
    System.out.println("    --command <name> : batch command: metamap, semrep, etc. (default: " + defaultCommand + ")");
  ...
  /**
   * If supplied value string is null or empty then display usage message and exit
   * program.
   * 
  ...
  public String processor(String args[], String apikey) {
    GenericObject myGenericObj = new GenericObject(apikey);
    return process_handler(args, myGenericObj);
  } 

  public String processor(String args[], String username, String password) {
    GenericObject myGenericObj = new GenericObject(username, password);
    return process_handler(args, myGenericObj);
  } 

  private String process_handler(String args[], GenericObject myGenericObj) {
    ...
    while (i < args.length) {
      if (args[i].charAt(0) == '-') {
        ...
        } else if (args[i].equals("--singleLinePMID") || args[i].equals("--singleLineDelimitedInputWithPMID")) {
